...
// DefaultRetryPolicy provides a default callback for WebSocket connection retries,
// which will retry on connection errors and WebSocket close errors (except normal closure).
func DefaultRetryPolicy(ctx context.Context, attemptNum int, err error) (bool, error) {
	...
	// Check for normal WebSocket closure - don't retry
...
// DefaultBackoff provides exponential backoff based on the attempt number and limited
...
func DefaultConfiguration() ClientConfiguration {
	...
	return ClientConfiguration{
		...
		WSBackoff:      DefaultBackoff,
		WSCheckRetry:   DefaultRetryPolicy,
	}
}
...
type ClientConfiguration struct {
	...
	//
	// Default: DefaultBackoff (exponential backoff)
	WSBackoff WSBackoff
